...
<?php
	...
	if (!empty($content['field_org_partner_type'])){
		$partner_type = $field_org_partner_type[0]['value'];
	}
	
	if (!empty($content['field_ecm_type'])){
		$ecocommunity_member_type = $field_ecm_type[0]['value'];
	} else {
	    $ecocommunity_member_type = "unknown" ;
	}
	...
	//print "TEMP: Partner Type:" . $partner_type  . "<br/>";
	
	// Wrap partner types in a class
	$partner_type = $field_org_partner_type[0]['value'];
	//$partner_type = $variables['field_org_partner_type'][0]['value'] ;
	if ($partner_type == "Basic Listing")
	...
	elseif($partner_type= "Partner Plus")
		...
		print 'div class="class-partner-type-unspecified"';  /* something weird here */
	}
?>   

  <h2 class="title org-title"><?php print $title; ?></h2>
<?php  
	if ($partner_type == "Partner" or $partner_type == "Partner Plus"){
		print render($content['field_org_logo']);
	}	
	hide ($content['field_org_logo']);
	//print render($title) ;
    /*<h2 class="title org-title" <?php print $title_attributes; ?>><?php print $title; ?></a></h2> */

	/* if basic - hide some fields */
	if ($partner_type == "Partner" or $partner_type == "Partner Plus"){
	...
	print render($content['field_org_affiliations']);
	
	hide($content['field_org_affiliations']);
	hide($content['field_contact_twitter']);
	hide($content['field_contact_linkedin']);
	hide($content['field_contact_youtube']);
	hide($content['field_contact_facebook']);

		

  /* for admin - show some private fields */
